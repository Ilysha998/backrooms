...
## Stamina drain rate per second when sprinting.
...
func handle_stamina(delta):
	...
	if STAMINA_BAR:
		STAMINA_BAR.value = current_stamina
		if state == "sprinting" or (state != "sprinting" and current_stamina < max_stamina):
...
func enter_normal_state():
	...
	if prev_state == "sprinting":
		stamina_delay_timer = stamina_regeneration_delay
